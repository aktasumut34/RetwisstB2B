<template>
  <div class="mainContent flex h-full flex-1 flex-col">
    ...
    <div
      class="flex max-h-screen flex-1 overflow-y-auto px-8 py-4 lg:max-h-[60%]"
    >
      <div
        class="grid w-full grid-cols-1 items-stretch justify-center gap-4 lg:grid-cols-3"
      >
        <div class="grid grid-rows-2 gap-4">
          ...
          >
            ...
            <div class="flex flex-col gap-2">
              <template v-if="product.ProductOptions.length > 0">
                <div v-for="option in product.ProductOptions">
                  <div
                    class="flex w-full justify-between rounded-lg bg-slate-200 px-4 py-2 text-left font-medium text-slate-800 transition-colors focus:outline-none focus-visible:ring"
                  >
              ...
              <template v-else>
                <span>This product doesn't have any options yet.</span>
              </template>
            </div>
          </div>
        </div>
        <div class="grid grid-rows-2 gap-4">
          ...
          >
            ...
            <div class="flex flex-col gap-2">
              <template v-if="product.ProductDescriptions.length > 0">
                <Disclosure
                  as="div"
                  v-for="description in product.ProductDescriptions"
                  v-slot="{ open }"
                >
                  <DisclosureButton
          ...
          >
            <div class="flex items-center justify-between">
              <span class="text-2xl">Category Links</span>
            </div>
            <div class="flex flex-col gap-2">
              <template v-if="product.Categories.length > 0">
                <div v-for="category in product.Categories">
                  ...
                  >
                    <span>{{
                      category.Category.CategoryDescriptions[0].name
                    }}</span>
                  </div>
                </div>
              </template>
              <template v-else>
                <span>This product doesn't have any category links yet.</span>
              </template>
            </div>
          </div>
        </div>
        ...
        >
          <div>
            <span class="text-2xl">Variants</span>
          </div>
          <div class="flex flex-col gap-3">
            <template v-if="product.ProductVariants.length > 0">
              <div
                v-for="variant in product.ProductVariants"
                class="flex gap-3 rounded-lg bg-white p-1"
              >
                <div class="w-16">
                  <img :src="variant.Images[0]?.image" />
                </div>
                <div class="flex flex-1 flex-col gap-1">
                  <span class="text-xl font-medium">{{ variant.model }}</span>
                  <span v-html="gethtml(variant)"> </span>
                </div>
              </div>
            </template>
            <template v-else>
              <span>This product doesn't have any variants yet.</span>
            </template>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="flex gap-3">
          <button
            ...
            @click="removeProduct"
          >
            <i class="pi pi-times-circle"></i> <span>Remove Product</span>
          </button>
        </div>
      </div>
    </div>
...
const fetchProduct = async () =>
  await $fetch(`http://localhost:3100/api/admin/products/${route.params.id}`, {
    method: 'GET',
    headers: {
...
} = await useAsyncData(`product-${route.params.id}`, fetchProduct);
...
const removeProduct = () => {
  if (product.value) {
    yesNoSwal
      .fire({
        title: 'Are you sure?',
        html: `Are you sure you want to remove the product <b>${product.value.model}</b>?`,
        icon: 'warning',
      ...
      .then(async (willDelete) => {
        if (willDelete.isConfirmed) {
          const {
            ...
            success: boolean;
          } = await $fetch(`http://localhost:3100/api/admin/products/remove`, {
            method: 'POST',
            body: {
              id: product.value.id
            },
          ...
          if (success) {
            singleButtonSwal
              .fire({
                title: 'Removed!',
                html: `The product <b>${product.value.model}</b> has been removed.`,
                icon: 'success'
              })
              .then(() => {
...
const gethtml = (v) =>
  v.ProductVariantOptionValues.map((item) => {
    return `<span class="font-extralight">${item.Option.name}</span>: ${item.value}`;
  }).join(', ');
definePageMeta({
  layout: 'admin'
});
useMeta({
  title: `Product: ${product.value?.model} - Admin - Retwisst`
});
</script>
