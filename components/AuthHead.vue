<template>
  <div>
    <div class="mb-10 flex flex-col gap-12">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          <ElementsUserdropdown class="hidden sm:inline-block" />
        </div>
      </div>
    ...
    <div
      ...
      :class="{
        'bg-opacity-40': cartPage,
        'pointer-events-none bg-opacity-0': !cartPage
      }"
    ></div>
    <TransitionRoot
      :show="cartPage"
      ...
      class="fixed right-0 top-0 z-[9999] flex h-screen w-[100%] max-w-[600px] flex-col gap-6 overflow-y-scroll bg-slate-100 py-12 px-8 transition-all lg:w-[50%]"
      enter="duration-300"
...
defineExpose({ cartPageOpenButton });
const setCartPage = (event) => {
  if (event.target === cartPageOpenButton.value.$el) {
    cartPage.value = true;
